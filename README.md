...
- **User Management**: review registrations, approve/suspend users, bind existing proxy `api-keys` to users, assign model access, and set monthly credit quotas. New proxy API keys are still created in the Config Panel `api-keys` list; the user page only binds or unbinds those existing keys.
