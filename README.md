...
- **User Management**: review registrations, approve/suspend users, assign one existing proxy `api-keys` entry to each user, assign model access, and set monthly credit quotas. New proxy API keys are still created in the Config Panel `api-keys` list; unbinding in the user page revokes the current authorization while preserving assignment and usage history.
